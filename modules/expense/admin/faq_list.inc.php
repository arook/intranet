...
$query = mysql_query ("select * from msg_bd where reid=0") ;
...
$query=mysql_query("select * from msg_bd");
...
<tr>
	<td>
		<? if ($rowsize>0) { ?>
		<input type="button" value="回覆內容" onClick="CheckAction('modify')">　
...
<tr align="center" bgcolor="C6C7E7" height="22">
	...
	<td width=100 background="images/back_99.gif">姓 名</td>
	<td background="images/back_99.gif">問 題</td>
</tr>
...
for ($i=0;$i<count($rowdata);$i++) {
	...
	<tr bgcolor="<? echo (($i+1)%2==0) ? "EFEFEF":"FFFFFF" ?>" align="center" height=22>
		...
		<td><input type="checkbox" name="nu" value="<? echo $rowdata[$i][serial] ?>"></td>
		<td><? echo $rowdata[$i]['name'] ?></td>
		<td align="left">&nbsp;<? echo $rowdata[$i]['title'] ?>&nbsp;:&nbsp;<? echo $rowdata[$i]['content'] ?></td>
	</tr>
