...
$query = mysql_query ("select * from pro_sel where type ='".$sort."'") ;
...
$query=mysql_query("select * from pro_sel where type ='".$sort."' order by name_eng desc limit ".$limitnum.",".$splitnum);
...
<?
		$query1=mysql_query("select * from pro_item where nu = '".$sort."'");
...
<tr>
	...
	<td align="right">
		...
		<?$query=mysql_query("select * from pro_item order by nu");
		while ($row = mysql_fetch_array($query))
		{?>
