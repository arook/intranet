...
$query = mysql_query ("select * from download where sort='".$sort."'") ;
...
$query=mysql_query("select * from download where sort='".$sort."' order by sn desc limit ".$limitnum.",".$splitnum);
...
<?
		mysql_connect ($dbshost, $dbsuser, $dbspass);
		mysql_select_db ($dbsname) or die( "Unable to select database"); 
		$query=mysql_query("select * from dwld order by 'nu'");
		while ($row = mysql_fetch_array($query))
		{
?>
		<option <? if ($sort==$row[product]) echo selected ?> value='<? echo $row[product];?>'><? echo $row[product];?></option>;
<?
	}
?>
		</select>
...
<tr align="center" bgcolor="C6C7E7" height="22">
	...
	<td width=60 background="images/back_99.gif">大小</td>
	<td background="images/back_99.gif">驅 動 程 式 名 稱</td>
</tr>
