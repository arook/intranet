...
define("_MD_AM_LOADINGIMG", "顯示「正在載入…」圖片？");
...
define("_MD_AM_REMEMBER", "為了讓系統能正常修改檔案，請將該檔案的屬性設為666");
...
define("_MD_AM_AVATARALLOW", "允許上傳圖像");
define("_MD_AM_AVATARMP", "最少發佈文章數");
define("_MD_AM_AVATARMPDSC", "只有發表超過該數目的會員才可以上傳圖像。");
define("_MD_AM_AVATARW","圖像最大寬度（像素）");
define("_MD_AM_AVATARH","圖像最大高度（像素）");
define("_MD_AM_AVATARMAX","圖像檔案大小（位元組）");
define("_MD_AM_AVATARCONF","自訂圖像設定");
...
define('_MD_AM_GENERAL', '基本參數設定');
define('_MD_AM_USERSETTINGS', '會員管理設定');
...
define('_MD_AM_BADIPS', '輸入要被禁止的 IP 位址。<br />請用 <strong>|</strong> 分隔關鍵詞，不區分大小寫，可使用 regex。');
...
define('_MD_AM_THEMEFILE', '從 /themes/yourtheme/templates 更新模組的樣板檔案');
define('_MD_AM_THEMEFILEDSC', '啟用這個選項後，如果 /themes/yourtheme/templates 有了更新，模組的 .html 樣板檔案將自動更新，而不再繼續使用快取的舊檔案，網站正式執行後，請關閉該功能。');
...
define("_MD_AM_LDAP_USETLS_DESC", "使用 TLS（Transport Layer Security-傳輸層安全）連接，TLS 使用標準的 389 連接埠<BR>".
                                  "並且 LDAP 的版本為3。");
...
define("_MD_AM_LDAP_FILTER_PERSON_DESC", "使用者搜尋會員的特殊 LDAP 過期器，@@loginname@@ 被取代成為使用者的登入名，如果你不清楚，請務必留空！" .
