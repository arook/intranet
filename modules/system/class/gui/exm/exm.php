    ...
    function header()
    {
        ...
        if (empty($xoopsModule) || 'system' == $xoopsModule->getVar('dirname', 'n')) {
            ...
            foreach (array_keys($mod_options) as $item) {
                ...
                $mod_options[$item]['icon'] = empty($mod_options[$item]['icon']) ? '' : file_exists(XOOPS_ROOT_PATH . "/modules/system/class/gui/exm/" . $mod_options[$item]['icon'])? XOOPS_URL . "/modules/system/class/gui/exm/" . $mod_options[$item]['icon']:XOOPS_URL . "/modules/{$moddir}/" . $mod_options[$item]['icon'];
            }
        }
