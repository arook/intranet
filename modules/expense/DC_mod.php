...
$apply_fee_Menu .="<option value=$row_sel[fee_nu]";
if($row_sel[fee_nu]==$row_re[apply_fee]){$apply_fee_Menu .=" selected";}
$apply_fee_Menu .=">$row_sel[fee_name]</option>";
}?>
...
<?$pro_sel = mysql_query ("select * from expense_fee where fee_nu = '".$row_re[apply_fee]."' and item_nu = '".$row_re[apply_fee_item]."'");
	$row_sel = mysql_fetch_array($pro_sel);
	echo "<option value=".$row_sel[fee_nu].">".$row_sel[item_name]."</option>"?>
</select>
