 ...
 * @version         $Id: functions.php 3898 2009-11-17 20:41:44Z trabis $
 */
...
function xoops_loadLanguage($name, $domain = '', $language = null)
{
     ...
     */
    if ($name == 'pagetype') {
        $name = xoops_getPageOption('pagetype');
    }
    /**
 ...
 * @return
 */
function xoops_getPageOption($option)
{
    $ret = '';
    if (isset($GLOBALS['xoopsOption']['pagetype'])) {
        $ret = preg_replace("/[^a-zA-Z0-9_]/", '', $GLOBALS['xoopsOption']['pagetype']);
    }
    return $ret;
}
 ...
 * @return
 */
function xoops_getConfigOption($option, $type = 'XOOPS_CONF', $allowed = array())
{
    ...
    if ($configs) {
        $cleaned = array();
        foreach ($configs as $key => $clean) {
            $cleaned[$key] = preg_replace('/[^a-zA-Z0-9_]/', '', $clean);
        }
        if (isset($cleaned[$option])) {
            $ret = $cleaned[$option];
        }
    }
 ...
 * Only alhpa and numumerical allowed
 *
 * @param mixed $option
 * @return
 */
function xoops_setConfigOption($option, $new = '')
{
    if (isset($GLOBALS['xoopsConfig'][$option]) && !is_null($new)) {
        $GLOBALS['xoopsConfig'][$option] = preg_replace('/[^a-zA-Z0-9_]/', '', $new);
    }
}
 ...
 * @return
 */
function xoops_getModuleOption($option, $dirname = '', $allowed = array())
{
    ...
    if ($module) {
        ...
        $cleaned = array();
        foreach ($moduleConfig as $key => $clean) {
            $moduleConfig[$key] = preg_replace('/[^a-zA-Z0-9_]/', '', $clean);
        }
